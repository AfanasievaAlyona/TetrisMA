...
namespace Tetris
{
    public partial class Tetris : Form
    {
        ...
        public void Init()
        {
            ...
            label3.Text = "Lines: " + linesRemoved;
        ...
        public void ShowNextShape(Graphics e)
        {
            for (int i = 0; i < currentShape.sizeNextMatrix; i++)
            {
                for (int j = 0; j < currentShape.sizeNextMatrix; j++)
                {
                    ...
                    {
                        e.FillRectangle(Brushes.Red, new Rectangle(350 + j * (size) + 1, 50 + i * (size) + 1, size - 1, size - 1));
                    }
                    if (currentShape.nextMatrix[i, j] == 2)
                    {
                        e.FillRectangle(Brushes.Yellow, new Rectangle(350 + j * (size) + 1, 50 + i * (size) + 1, size - 1, size - 1));
                    }
                    if (currentShape.nextMatrix[i, j] == 3)
                    {
                        e.FillRectangle(Brushes.Green, new Rectangle(350 + j * (size) + 1, 50 + i * (size) + 1, size - 1, size - 1));
                    }
                    if (currentShape.nextMatrix[i, j] == 4)
                    {
                        e.FillRectangle(Brushes.Blue, new Rectangle(350 + j * (size) + 1, 50 + i * (size) + 1, size - 1, size - 1));
                    }
        ...
        public void ResetArea()
        {
            for (int i = currentShape.y; i < currentShape.y + currentShape.sizeMatrix; i++)
            {
                for (int j = currentShape.x; j < currentShape.y + currentShape.sizeMatrix; j++)
                {
                    if (i >= 0 && j >= 0 && i < 25 && j < 10)
                    {
                        if (currentShape.matrix[i - currentShape.y, j - currentShape.x] != 0)
                        {
                            map[i, j] = 0;
        ...
        public void DrawGrid(Graphics g)
        {
            ...
            for (int i = 0; i <= 10; i++)
            {
                g.DrawLine(Pens.Black, new Point(50 + i * size, 50), new Point(50 + i * size, 50 + 25 * size));
            }
        }
        ...
        private void button1_Nachalo_Click(object sender, EventArgs e)
        {
            timer1.Tick -= new EventHandler(update);
            timer1.Stop();
            Init();
        }
        private void button2_Pusk_Click(object sender, EventArgs e)
        {
            var pressedButton = sender as ToolStripMenuItem;
            if (timer1.Enabled)
            {
                pressedButton.Text = "Пуск";
                timer1.Stop();
            }
            else
            {
        ...
        private void button3_Vihod_Click(object sender, EventArgs e)
        {
            Close();
        }
    }
